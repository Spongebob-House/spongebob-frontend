...
async function findById(userid, success, fail) {
  api.defaults.headers['access-token'] = Vue.$cookies.get('access-token');
  await api.get(`/user/info/${userid}`).then(success).catch(fail);
}
// access token 재 발급
async function tokenRegeneration(user, success, fail) {
  api.defaults.headers['refresh-token'] = Vue.$cookies.get('refresh-token'); //axios header에 refresh-token 셋팅
